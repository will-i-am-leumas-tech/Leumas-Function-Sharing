...
function getExecutionForEntry({ entryType, modulePath, exportName, rootDir }) {
...
async function indexProject({ rootDir, outFile } = {}) {
  ...
  const packageRunnableMeta = getPackageRunnableMeta(targetRoot, allFiles);
  ...
  for (const sdkEntry of detectSdkEntrypoints({ rootDir: targetRoot, allFiles })) {
    entries.push({
  ...
  for (const filePath of allFiles) {
    ...
    if (isJavaScript) {
      for (const route of detectExpressRoutes({ filePath, source })) {
        const routeEntry = createExpressRouteEntry({ rootDir: targetRoot, filePath, relativePath, route });
    ...
    for (const exp of exports) {
      ...
      entries.push({
        ...
        execution: getExecutionForEntry({ entryType, modulePath: filePath, exportName, rootDir: targetRoot }),
      });
